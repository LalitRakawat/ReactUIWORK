...
const Sider = () => {
    ...
    const subMenuElement = (dataitm) => {
        return dataitm.hasChild === 0 
              ? (  <Menu.Item key={dataitm.menuId}> {dataitm.menuText} </Menu.Item>) 
              : (  <SubMenu key={ dataitm.menuId } title={dataitm.menuText}>
                      {
                        dataitm.children 
                        ? (dataitm.children.map((itm) => { return <>{ subMenuElement(itm) }</>;})) 
                        : ( <Menu.Item key={dataitm.menuId}> {dataitm.menuText} </Menu.Item>)
                      } 
                    </SubMenu>
                );
    ...
    return (
        <Menu mode="inline" openKeys={openKeys} onOpenChange={onOpenChange} theme="light">
            {
                            menuStack.children.map((itm) => {
                                if (itm.hasChild) {
                                    return subMenuElement(itm);
                                } 
                                else 
                                {
